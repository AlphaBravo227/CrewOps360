...
Please log in to https://dashboard.crewops360.com/ and navigate to Track Bidding to submit your selections.  Detailed instructions are linked at the top of the Track Bidding section.
...
This notification was automatically generated by the Boston MedFlight CrewOps360 system.
    ...
    def create_needs_swap_decision_content(self, staff_name, track_name, decision, details=None):
        """
        Create email subject/body telling a staff member (and the admin recipients)
        that their Needs Swap offer was approved, declined, or that a decision on it
        has been rescinded.

        The admin's own review note is deliberately NOT included — that stays in the
        admin views for reference. What goes out is the decision itself and, when
        approved, a summary of the change that was actually applied to their track;
        when rescinded, what was put back.

        Args:
            staff_name (str): Staff member who submitted the offer
            track_name (str): Bid track/cycle name
            decision (str): 'approved', 'declined' or 'rescinded'
            details (dict, optional): need_day, need_period, give_up_day, give_up_period,
                hypothetical_base, hypothetical_base_rank, relaxations (list of str),
                superseded (int), reviewed_by; plus previous_status, reverted and
                reopened on a rescission
        ...
        need_shift = self._shift_phrase(details.get('need_day'), details.get('need_period'))
        give_up_shift = self._shift_phrase(details.get('give_up_day'), details.get('give_up_period'))
        reviewed_by = details.get('reviewed_by') or 'Management'

        if approved:
            subject = f"Boston MedFlight - Swap Approved: {staff_name} ({track_name})"

            change_lines = [
                f"- Picked up: {need_shift}",
                f"- Gave up: {give_up_shift}",
            ]
            hypothetical_base = details.get('hypothetical_base')
            if hypothetical_base:
                rank = details.get('hypothetical_base_rank')
                rank_text = f"#{rank} on your list" if rank is not None else "unranked on your list"
                change_lines.append(
                    f"- Hypothetical shift: {hypothetical_base} ({rank_text}) — the same base "
                    "you were shown when you offered, which no one already working that day "
                    "has to drop below their own ranking for"
                )
            relaxations = details.get('relaxations') or []
            if relaxations:
                change_lines.append(
                    f"- Adjusted for this cycle: {'; '.join(relaxations)} — covering the need is "
                    "worth more than the minimum, so there is nothing to make up elsewhere"
                )
            superseded = details.get('superseded') or 0
            if superseded:
                change_lines.append(
                    f"- {superseded} other option{'s' if superseded != 1 else ''} you offered for "
                    "that same need " + ("were" if superseded != 1 else "was") + " superseded"
                )

            body = f"""
Boston MedFlight Track Needs - Swap Approved
...
This swap has been approved and applied to {staff_name}'s track for {track_name}.

Changes made:
{chr(10).join(change_lines)}

The updated track is live now — log in to https://dashboard.crewops360.com/ to see it.

This notification was automatically generated by the Boston MedFlight CrewOps360 system.
            """
        elif decision == 'rescinded':
            previous = details.get('previous_status') or 'decided'
            subject = f"Boston MedFlight - Swap Decision Reversed: {staff_name} ({track_name})"

            if details.get('reverted'):
                what_changed = f"""Their track has been put back the way it was before the approval:

- Working again: {give_up_shift}
- Off again: {need_shift}, which goes back to being an open need"""
                reopened = details.get('reopened') or 0
                if reopened:
                    what_changed += (
                        f"\n- {reopened} other option{'s' if reopened != 1 else ''} offered for that "
                        f"same need {'are' if reopened != 1 else 'is'} pending again")
            else:
                what_changed = ("Nothing on their track changed when this was "
                                f"{previous}, so there is nothing to put back.")

            body = f"""
Boston MedFlight Track Needs - Swap Decision Reversed

Staff Member: {staff_name}
Bid Track: {track_name}
Reversed by: {reviewed_by}
Timestamp: {timestamp}

The earlier decision on this swap offer has been rescinded — it was {previous}, and that
no longer stands:

- Offered to pick up: {need_shift}
- Offered to give up: {give_up_shift}

{what_changed}

The offer is back under review with everything else still pending, and you will be emailed
again once it is decided. Nothing further is needed from you.

You can see where every offer stands at https://dashboard.crewops360.com/ under Track Needs.

This notification was automatically generated by the Boston MedFlight CrewOps360 system.
            """
        else:
            subject = f"Boston MedFlight - Swap Not Selected: {staff_name} ({track_name})"
            body = f"""
Boston MedFlight Track Needs - Swap Not Selected
...
The following swap offer was reviewed and not selected:

- Offered to pick up: {need_shift}
- Offered to give up: {give_up_shift}

Nothing has changed on {staff_name}'s track for {track_name} — it stands exactly as it was
bid. Any other options still pending are unaffected and remain under review.

You can see the status of every offer at https://dashboard.crewops360.com/ under Track Needs.
...
This notification was automatically generated by the Boston MedFlight CrewOps360 system.
    ...
    def send_needs_swap_decision_notification(self, staff_name, staff_email, track_name,
                                              decision, details=None):
        """
        Email the staff member and the admin recipients that a Needs Swap offer was
        approved or declined, or that a decision on it was rescinded. The admin's
        review note is never included.

        When no email address is on file for the staff member the admins are still
        notified, and the returned message says the individual could not be reached.

        Args:
            staff_name (str): Staff member who submitted the offer
            staff_email (str): Their email address, or None/blank if not on file
            track_name (str): Bid track/cycle name
            decision (str): 'approved', 'declined' or 'rescinded'
            details (dict, optional): See create_needs_swap_decision_content()
        ...
        staff_email = (staff_email or "").strip()
        staff_reachable = bool(staff_email) and bool(_EMAIL_RE.match(staff_email))

        try:
            subject, body = self.create_needs_swap_decision_content(
                staff_name, track_name, decision, details)

            recipients = [staff_email] if staff_reachable else []
            if self.admin_email and self.admin_email not in recipients:
                recipients.append(self.admin_email)
            if self.notification_recipients:
                for email in self.notification_recipients:
            ...
            success = self.send_email(recipients, subject, body)
            if not success:
                return (False, "Failed to send swap decision notification")

            if staff_reachable:
                return (True, f"Emailed {staff_name} ({staff_email}) and admins.")
            return (False, f"No email address on file for {staff_name} — admins were "
                           "notified, but they were not.")

        except Exception as e:
            return (False, f"Error sending swap decision notification: {str(e)}")

    @staticmethod
    def _shift_phrase(day_label, period):
        """'Sun B 4' + 'Night' -> 'Night on Sun B 4', for use in email bodies."""
        if not day_label:
            return "Unknown shift"
        return f"{period} on {day_label}" if period else str(day_label)
...
def send_needs_swap_decision_notification(staff_name, staff_email, track_name, decision, details=None):
    """
    Convenient function to email a staff member and the admins that their Needs Swap
    offer was approved (with a summary of the change applied), declined, or that a
    decision on it was rescinded (with a summary of what went back).

    The admin's review note is never included — it stays in the admin views.
    ...
    Args:
        ...
        decision (str): 'approved', 'declined' or 'rescinded'
        details (dict, optional): need_day, need_period, give_up_day, give_up_period,
            hypothetical_base, hypothetical_base_rank, relaxations, superseded, reviewed_by

    Returns:
        tuple: (success, message)
    """
    return email_notifier.send_needs_swap_decision_notification(
        staff_name, staff_email, track_name, decision, details)
