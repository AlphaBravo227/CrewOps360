...
Which staff a training class is for is not an attribute of the staff row at all: it
comes from the named groupings in modules/staff_groupings.py, which a staff member can
belong to any number of. This module owns the roster; that one owns the groupings drawn
over it.
...
# they must not block a delete the way a track or an enrollment does — grouping
# membership is part of the roster entry, so it goes when the roster entry goes.
_MEMBERSHIP_TABLES = {'staff_grouping_members'}
...
def initialize_staff_tables():
    ...
    try:
        ...
        # The groupings are built over this roster, so their tables come up with it —
        # and the first run migrates the old education_group / or_group columns into
        # them. Imported here rather than at the top of the module because
    ...
    if staff_table_exists():
        try:
            try:
                ...
                # the requirements columns. Run the migration and read again rather
...
def get_roster_issues(include_inactive=False):
    ...
    Returns:
        ...
        'missing_email', and 'missing_grouping' (staff who work tracks but are in no
        grouping, so no class assigned by grouping reaches them).
    """
    records = get_all_staff(include_inactive=include_inactive)

    # Imported here rather than at the top of the module: staff_groupings reads the
    # roster through this one, so the dependency only runs in this direction on demand.
    try:
        from . import staff_groupings
        ungrouped = set(staff_groupings.get_ungrouped_staff(
            include_inactive=include_inactive))
    except Exception as e:
        print(f"Error reading the groupings for the roster issues: {e}")
        ungrouped = set()
    ...
    return {
        ...
        # Classes are assigned by grouping, so a staff member who works tracks and is
        # in none of them gets nothing assigned. Being in no grouping is normal for
        # management and the non-clinical roles, so only track-working staff are
        # flagged.
        'missing_grouping': [r['staff_name'] for r in records
                             if bids_on_tracks(r) and r['staff_name'] in ungrouped],
    }
...
def _staff_reference_columns(include_memberships=True):
    """
    ...
    Args:
        include_memberships (bool): keep the roster-attribute tables (_MEMBERSHIP_TABLES).
            A rename has to rewrite them; counting what would be orphaned by a delete
            must not, or being in a grouping would block deleting a staff member.
    """
    references = []
    try:
        cursor = _get_conn().cursor()
        ...
        for table in tables:
            if table in _OWN_TABLES or table.startswith('sqlite_'):
                continue
            if not include_memberships and table in _MEMBERSHIP_TABLES:
                continue
...
def count_staff_references(staff_name):
    ...
    try:
        cursor = _get_conn().cursor()
        for table, column in _staff_reference_columns(include_memberships=False):
            cursor.execute(
