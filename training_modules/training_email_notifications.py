...
To review enrollments, please access the Training & Events module in CrewOps360.
"""
...
def send_summer_leave_notification(staff_name, role, week_start_date, week_end_date,
    ...
    Args:
        ...
        week_start_date (str): Start date of selected week (YYYY-MM-DD)
    ...
    Returns:
        tuple: (success, message)
    """
    if not training_email_notifier.configured:
        return (False, "Email notifications not configured")
    ...
    try:
        ...
        week_start = dt.strptime(week_start_date, '%Y-%m-%d')
        week_end = dt.strptime(week_end_date, '%Y-%m-%d')
        week_display = f"{week_start.strftime('%B %d')}-{week_end.strftime('%d, %Y')}"

        # Create subject
        subject = f"Boston MedFlight Summer Leave Selection: {staff_name} - {role}"

        # Create body
        body = f"""
Boston MedFlight Summer Leave Time Selection Notification

ACTION: SUMMER LEAVE SELECTION
Staff Member: {staff_name}
Role: {role}
...
"""
        ...
        if total_selected is not None and role_cap is not None:
            body += f"""
Current Week Selection Status:
Total {role}s Selected for This Week: {total_selected}/{role_cap}
"""

        body += """
---
This notification was automatically generated by the Boston MedFlight Summer Leave Time system.

To review all selections, please access the Summer Leave admin interface.
"""
        ...
        if training_email_notifier.notification_recipients:
            for email in training_email_notifier.notification_recipients:
        ...
        # Send the email
        success = training_email_notifier.send_email(recipients, subject, body)

        if success:
            return (True, "Summer leave notification sent successfully.")
        else:
            return (False, "Failed to send email notification")
