...
class EnrollmentManager:
    ...
    def _is_non_mgmt_medic(self, staff_name):
        ...
        Returns True if Role=MEDIC and MGMT is not set

        Role and MGMT come from the staff database via the Excel handler.
        """
        try:
            role = self.excel.get_staff_role(staff_name)
            if not role or str(role).strip().upper() != "MEDIC":
                return False
            return not self.excel.is_management(staff_name)
    ...
    def _check_weekly_enrollment_limit(self, staff_name, class_date, class_name=None):
        ...
        try:
            ...
            if len(week_class_names) >= weekly_limit:
                existing_class = week_class_names[0]
                limit_word = "two classes" if weekly_limit == 2 else "one class"
                error_message = (
                    f"Cannot enroll - you are already enrolled in {existing_class} this week "
                    f"and are limited to {limit_word} per week."
                )
                return False, error_message, existing_class
    ...
    def enroll_staff(self, staff_name, class_name, class_date, role='General',
        ...
        for date in enrollment_dates:
            can_enroll, limit_error, existing_class = self._check_weekly_enrollment_limit(staff_name, date, class_name)
            if not can_enroll:
    ...
    def _session_options_for_location(self, class_name, class_date, option=None,
        ...
        if is_multi_session and session_length:
            ...
            # Extract time information for single-session display
